"""Step 04: Prepare data - scaling and windowing (NO PCA)."""
import numpy as np
import pandas as pd
...
train_scaled_df = pd.DataFrame(x_scaler.transform(x_train_raw), index=train_df.index, columns=feature_cols)
val_scaled_df = pd.DataFrame(x_scaler.transform(x_val_raw), index=val_df.index, columns=feature_cols)
test_scaled_df = pd.DataFrame(x_scaler.transform(x_test_raw), index=test_df.index, columns=feature_cols)
...
print("Prepared scaled datasets for sweep experiments (NO PCA).")
